...
$sql = " SELECT us.*, i.*, t.* FROM tb_usuarios AS t LEFT JOIN usuarios_seguidores AS us ON (us.id_usuario = $id_usuario AND t.id = us.seguindo_id_usuario) LEFT JOIN imagens AS i ON (t.id = i.id_usuario) WHERE t.id = seguindo_id_usuario AND t.id <> $id_usuario ";
